...
void setup () 
{
  ...
  void draw() 
  {
    ...
    {
      stepX = -5;
      println("bounce right side");

      //check collision with robot player
      if (robotY < posY && posY< robotY + 50)
      {
        //score2++; man kan også gør det sådan
        score2 = score2 + 1;
        println("hit");
      } else // robot misses
      {
    ...
    robotY = robotY*0.99 + 0.01 * (posY -10); // sætter en hastighed
    rect(width-20, robotY, 20, 100, 7);

    text( score1, 200, 30); // hvad der skal stå på skærmen (text)
...
print("x:"); 
  ...
  void resetBall() // vi kan sætte resetball ind på posX = width/2; og  posY = height/2; i hele programmet
